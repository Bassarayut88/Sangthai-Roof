...
function setDrawingMode(active) {
    ...
    let btnTouchLine = document.getElementById('touchBtnLine');
    if (btnTouchLine) {
        btnTouchLine.classList.toggle('active', polyDrawingMode);
        btnTouchLine.innerHTML = polyDrawingMode ? '✏️ กำลังวาด' : '✏️ วาด (Line)';
    }
...
// ============================================================
// TOUCH ENGINE STATE (Mobile / Tablet / iPad Touch Tracking)
// ============================================================
let cadTouchState = {
    active: false,
    startTime: 0,
...
function getTouchCanvasPos(canvas, touch) {
    if (!canvas || !touch) return { x: 0, y: 0, clientX: 0, clientY: 0 };
...
function polyCanvasPos(evt) {
    ...
    let cx = evt.clientX;
    let cy = evt.clientY;
    if (cx === undefined && evt.touches && evt.touches.length > 0) {
        cx = evt.touches[0].clientX;
        cy = evt.touches[0].clientY;
    } else if (cx === undefined && evt.changedTouches && evt.changedTouches.length > 0) {
...
function initPolyEditor() {
    ...
    // ---------- Touch Engine for Inline Canvas ----------
    c.addEventListener('touchstart', function(evt) {
        if (evt.touches.length === 2) {
            evt.preventDefault();
            polyTouchState.isPinching = true;
            polyTouchState.active = false;
            draggingPointIndex = -1;
            isDraggingStartSeam = false;
            const t1 = evt.touches[0];
            const t2 = evt.touches[1];
            polyTouchState.pinchDist = Math.hypot(t1.clientX - t2.clientX, t1.clientY - t2.clientY);
            polyTouchState.pinchScale = polyView.scale;
            polyTouchState.pinchMid = { x: (t1.clientX + t2.clientX) / 2, y: (t1.clientY + t2.clientY) / 2 };
            polyTouchState.pinchOx = polyView.ox;
            polyTouchState.pinchOy = polyView.oy;
            return;
        }
        ...
        if (evt.touches.length === 1) {
            ...
            const pos = getTouchCanvasPos(c, touch);
            lastMousePos = pos;
            polyTouchState.active = true;
            polyTouchState.isPinching = false;
            polyTouchState.startTime = Date.now();
            polyTouchState.startScreen = { x: pos.x, y: pos.y };
            polyTouchState.currentScreen = { x: pos.x, y: pos.y };
            polyTouchState.moved = false;

            // 1. Dragging Seam (Touch hit radius = 24px)
            if (polygonPoints.length >= 3) {
                let seamX = (typeof polyStartSeamX === 'number' && !isNaN(polyStartSeamX)) ? polyStartSeamX : (polygonPoints[0]?.x || 0);
                let sSeamX = seamX * polyView.scale + polyView.ox;
                if (Math.abs(pos.x - sSeamX) < 24) {
                    isDraggingStartSeam = true;
            ...
            // 2. Dragging Point (Touch hit radius = 24px)
            for (let i = 0; i < polygonPoints.length; i++) {
                let s = polyToScreen(polygonPoints[i]);
                if (Math.hypot(pos.x - s.x, pos.y - s.y) < 24) {
                    if (i === 0 && polygonPoints.length >= 3 && polyDrawingMode) continue;
                    draggingPointIndex = i;
                    if (navigator.vibrate) navigator.vibrate(20);
                    redrawPolyEditor();
            ...
            // 3. Aim / Live Rubberband Line
            if (polyDrawingMode || polyPickStartMode || cadArcState.active || cadGuideState.active || cadMeasureState.active || cadCalibrateState.active) {
                polyHover = polyScreenToWorld(pos.x, pos.y, false);
            }
            redrawPolyEditor();
        }
    }, { passive: false });
    ...
    window.addEventListener('touchmove', function(evt) {
        if (!polyTouchState.active && !polyTouchState.isPinching) return;
        const c = document.getElementById('polyDrawCanvas');
        if (!c) return;

        // 2-Finger Multi-Touch: Pan & Pinch Zoom
        if (evt.touches.length === 2 && polyTouchState.isPinching) {
            evt.preventDefault();
            const t1 = evt.touches[0];
            const t2 = evt.touches[1];
            ...
            if (polyTouchState.pinchDist > 5) {
                const rect = c.getBoundingClientRect();
                const zoomFactor = curDist / polyTouchState.pinchDist;
                const newScale = Math.max(5, Math.min(polyTouchState.pinchScale * zoomFactor, 300));

                const wx = (polyTouchState.pinchMid.x - rect.left - polyTouchState.pinchOx) / polyTouchState.pinchScale;
                const wy = (polyTouchState.pinchOy - (polyTouchState.pinchMid.y - rect.top)) / polyTouchState.pinchScale;

                polyView.scale = newScale;
                polyView.ox = (curMid.x - rect.left) - wx * newScale;
                polyView.oy = (curMid.y - rect.top) + wy * newScale;
                redrawPolyEditor();
            }
            return;
        }

        if (evt.touches.length === 1 && polyTouchState.active) {
            evt.preventDefault();
            const touch = evt.touches[0];
            const pos = getTouchCanvasPos(c, touch);
            lastMousePos = pos;
            ...
            if (Math.hypot(pos.x - polyTouchState.startScreen.x, pos.y - polyTouchState.startScreen.y) > 6) {
                polyTouchState.moved = true;
            }
    ...
    window.addEventListener('touchend', function(evt) {
        if (polyTouchState.isPinching) {
            if (evt.touches.length === 0) polyTouchState.isPinching = false;
            return;
        }

        if (!polyTouchState.active) return;
        polyTouchState.active = false;
        ...
        const pos = polyTouchState.currentScreen;
        ...
        // 2. Guideline
        if (cadGuideState.active) {
            let p = polyScreenToWorld(pos.x, pos.y, false);
            handleGuidePointClick(p);
            return;
        }
        ...
        if (cadCalibrateState.active) {
            let wx = (pos.x - polyView.ox) / polyView.scale;
            let wy = (polyView.oy - pos.y) / polyView.scale;
            handleCalibratePointClick({ x: Math.round(wx * 1000) / 1000, y: Math.round(wy * 1000) / 1000 });
            return;
        }

        // 6. Not in Drawing Mode (Edit / Offset)
        if (!polyDrawingMode) {
            cadHoveredEdge = findHoveredEdge(pos.x, pos.y, polyToScreen);
            if (cadHoveredEdge >= 0 && cadHoveredEdge < polygonPoints.length && polygonPoints.length >= 2) {
                selectedEdgeForOffset = cadHoveredEdge;
        ...
        if (polygonPoints.length >= 3) {
            let first = polyToScreen(polygonPoints[0]);
            if (Math.hypot(pos.x - first.x, pos.y - first.y) < 24) {
                polyCloseShape();
                return;
            }
        }
...
function initCadStudioCanvas() {
    ...
    // ---------- Touch Engine for Fullscreen AutoCAD Canvas ----------
    canvas.addEventListener('touchstart', function(evt) {
        if (!cadStudioOpen) return;
        const rect = canvas.getBoundingClientRect();

        // 2-Finger Multi-Touch: Pan & Pinch to Zoom
        if (evt.touches.length === 2) {
            evt.preventDefault();
            cadTouchState.isPinching = true;
            cadTouchState.active = false;
            cadDraggingIndex = -1;
            cadDraggingStartSeam = false;
            cadDraggingBgImage = false;
            ...
            const t1 = evt.touches[0];
            const t2 = evt.touches[1];
            cadTouchState.pinchDist = Math.hypot(t1.clientX - t2.clientX, t1.clientY - t2.clientY);
            cadTouchState.pinchScale = cadView.scale;
            cadTouchState.pinchMid = { x: (t1.clientX + t2.clientX) / 2, y: (t1.clientY + t2.clientY) / 2 };
            cadTouchState.pinchOx = cadView.ox;
            cadTouchState.pinchOy = cadView.oy;
            return;
        }
        ...
        if (evt.touches.length === 1) {
            evt.preventDefault();
            const touch = evt.touches[0];
            const pos = getTouchCanvasPos(canvas, touch);
            cadMouseScreen = { x: pos.x, y: pos.y };
            cadTouchState.active = true;
            cadTouchState.isPinching = false;
            cadTouchState.startTime = Date.now();
            cadTouchState.startScreen = { x: pos.x, y: pos.y };
            cadTouchState.currentScreen = { x: pos.x, y: pos.y };
            cadTouchState.moved = false;

            // 1. Check Seam drag (Touch hit radius = 26px)
            if (polygonPoints.length >= 3) {
                let minX = Math.min(...polygonPoints.map(p => p.x));
                let seamX = (typeof polyStartSeamX === 'number' && !isNaN(polyStartSeamX)) ? polyStartSeamX : minX;
                let sSeamX = seamX * cadView.scale + cadView.ox;
                if (Math.abs(pos.x - sSeamX) < 26) {
                    cadDraggingStartSeam = true;
                    if (navigator.vibrate) navigator.vibrate(20);
                    redrawCadStudio();
                    return;
            ...
            // 2. Check Vertex drag (Touch hit radius = 26px)
            for (let i = 0; i < polygonPoints.length; i++) {
                let s = cadWorldToScreen(polygonPoints[i].x, polygonPoints[i].y);
                if (Math.hypot(pos.clientX - s.x - rect.left, pos.clientY - s.y - rect.top) < 26 || Math.hypot(pos.x - s.x, pos.y - s.y) < 26) {
                    if (i === 0 && polygonPoints.length >= 3 && polyDrawingMode) continue;
                    cadDraggingIndex = i;
                    cadDragStartPos = { x: polygonPoints[i].x, y: polygonPoints[i].y };
                    if (navigator.vibrate) navigator.vibrate(20);
            ...
            // 3. Check Blueprint drag
            if (!cadBgImage.locked && cadBgImage.img && !polyDrawingMode && !cadCalibrateState.active) {
                let world = cadScreenToWorld(pos.x, pos.y, false);
                if (world.x >= cadBgImage.x && world.x <= cadBgImage.x + cadBgImage.width &&
                    world.y <= cadBgImage.y && world.y >= cadBgImage.y - cadBgImage.height) {
                    cadDraggingBgImage = true;
                    cadBgDragStart = { mouseX: world.x, mouseY: world.y, imgX: cadBgImage.x, imgY: cadBgImage.y };
            ...
            // 4. Live line / Reticle Aim
            cadHover = cadCalculateSnapAndWorld(pos.x, pos.y, isShiftPressed || cadOrtho);
            updateCadStatusText();
            redrawCadStudio();
        }
    }, { passive: false });
    ...
    window.addEventListener('touchmove', function(evt) {
        if (!cadStudioOpen) return;
        if (!cadTouchState.active && !cadTouchState.isPinching) return;
        const canvas = document.getElementById('cadStudioCanvas');
        if (!canvas) return;

        // 2-Finger Multi-Touch: Pan & Pinch Zoom
        if (evt.touches.length === 2 && cadTouchState.isPinching) {
            evt.preventDefault();
            const t1 = evt.touches[0];
            const t2 = evt.touches[1];
            const curDist = Math.hypot(t1.clientX - t2.clientX, t1.clientY - t2.clientY);
            ...
            if (cadTouchState.pinchDist > 5) {
                const rect = canvas.getBoundingClientRect();
                const zoomFactor = curDist / cadTouchState.pinchDist;
                const newScale = Math.max(4, Math.min(cadTouchState.pinchScale * zoomFactor, 800));

                const wx = (cadTouchState.pinchMid.x - rect.left - cadTouchState.pinchOx) / cadTouchState.pinchScale;
                const wy = (cadTouchState.pinchOy - (cadTouchState.pinchMid.y - rect.top)) / cadTouchState.pinchScale;

                cadView.scale = newScale;
                cadView.ox = (curMid.x - rect.left) - wx * newScale;
                cadView.oy = (curMid.y - rect.top) + wy * newScale;
                redrawCadStudio();
            }
            return;
        }

        if (evt.touches.length === 1 && cadTouchState.active) {
            evt.preventDefault();
            const touch = evt.touches[0];
            const pos = getTouchCanvasPos(canvas, touch);
            cadMouseScreen = { x: pos.x, y: pos.y };
            ...
            if (Math.hypot(pos.x - cadTouchState.startScreen.x, pos.y - cadTouchState.startScreen.y) > 6) {
                cadTouchState.moved = true;
            }
            ...
            if (cadDraggingIndex >= 0 && cadDraggingIndex < polygonPoints.length) {
                ...
                if (isOrtho && cadDragStartPos) {
                    ...
                    let snap = cadCalculateSnapAndWorld(pos.x, pos.y, isShiftPressed || cadOrtho);
                    finalX = snap.x;
                    finalY = snap.y;
                }
            ...
            // Live Rubberband Line / Reticle Aim with OSNAP
            cadHover = cadCalculateSnapAndWorld(pos.x, pos.y, isShiftPressed || cadOrtho);
            updateCadStatusText();
            redrawCadStudio();
        }
    ...
    window.addEventListener('touchend', function(evt) {
        if (!cadStudioOpen) return;
        if (cadTouchState.isPinching) {
            if (evt.touches.length === 0) cadTouchState.isPinching = false;
            return;
        }

        if (!cadTouchState.active) return;
        cadTouchState.active = false;
        ...
        const pos = cadTouchState.currentScreen;
        cadMouseScreen = { x: pos.x, y: pos.y };
        ...
        // 5. Calibrate
        if (cadCalibrateState.active) {
            let raw = cadScreenToWorld(pos.x, pos.y, false);
            let finalPt = raw;
            if (cadCalibrateState.step === 2 && cadCalibrateState.pt1) {
                let isOrtho = isShiftPressed || cadOrtho;
                if (isOrtho) {
        ...
        // 6. Offset Selection
        if (cadOffsetSelectingEdge && cadHoveredEdge >= 0 && polygonPoints.length >= 3) {
            cadOpenPrompt('offset', cadHoveredEdge);
            return;
        }

        // 7. Selection / Edit Mode
        if (!polyDrawingMode) {
            if (cadHoveredEdge >= 0 && cadHoveredEdge < polygonPoints.length && polygonPoints.length >= 2) {
                selectedEdgeForOffset = cadHoveredEdge;
                cadOpenPrompt('edge_edit', cadHoveredEdge);
        ...
        // 8. Close Polygon
        if (polygonPoints.length >= 3 && (cadHover.snapType === 'firstPoint' || Math.hypot(pos.x - cadWorldToScreen(polygonPoints[0].x, polygonPoints[0].y).x, pos.y - cadWorldToScreen(polygonPoints[0].x, polygonPoints[0].y).y) < 26)) {
            polyCloseShape();
            return;
        }
...
function toggleCadOrtho() {
    ...
    let btnTouch = document.getElementById('touchBtnOrtho');
    if (btnTouch) btnTouch.classList.toggle('active', cadOrtho);
    updateCadStatusText();
    redrawCadStudio();
}
...
function toggleCadSnap() {
    ...
    let btnTouch = document.getElementById('touchBtnSnap');
    if (btnTouch) btnTouch.classList.toggle('active', cadSnap);
    redrawCadStudio();
}
...
function redrawCadStudio() {
    ...
    if (cadOverlay && polygonPoints.length >= 3) {
        ...
        let sheetPixelW = sheetW * cadView.scale;
        let stepSkip = (sheetPixelW < 22) ? Math.ceil(24 / Math.max(sheetPixelW, 1)) : 1;

        let visibleSheetIndex = 0;
        for (let k = leftK; k < rightK; k++) {
            ...
            if (bounds.length > 0.01) {
                ...
                // ลูกศรสองหัวสีเขียว (แสดงตัวเลขเฉพาะเมื่อมีพื้นที่พอ ไม่ให้ตัวเลขซ้อนทับกัน)
                let showLabel = (sheetPixelW >= 22) || (visibleSheetIndex % stepSkip === 0);
                let midX = (pTopL.x + pBotR.x) / 2;
                drawPolygonSheetArrow(ctx, midX, pBotR.y, pTopL.y, bounds.length.toFixed(2), '#00c800', isDark, showLabel);
                visibleSheetIndex++;
            }
        }
        ctx.restore();
        ...
        // ด้ามจับแนวเริ่มมุง (ดีไซน์กะทัดรัด ไม่บดบังรูปวาด)
        let badgeText = `📍 เริ่มมุง ${seamX.toFixed(2)}m`;
        ctx.font = 'bold 10px monospace';
        let btm = ctx.measureText(badgeText);
        let bw = btm.width + 12;
        let bh = 20;
        let by = Math.max(8, Math.min(h - 75, h - 35));
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.roundRect(sSeamX - bw / 2, by, bw, bh, 10);
        ctx.fill();
        ctx.stroke();
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(badgeText, sSeamX, by + bh / 2);
        ctx.restore();
    }
    ...
    // 6.7 Touch Precision Reticle & Floating HUD (เมื่อใช้นิ้วสัมผัส/ลากเส้นบน Mobile & iPad)
    if (cadTouchState.active && cadHover) {
        let sHover = cadWorldToScreen(cadHover.x, cadHover.y);
        ctx.save();

        // 1. Target Reticle Crosshair (เป้าเล็งวงแหวนพร้อมเข็มทิศรอบจุด Snap)
        ctx.strokeStyle = cadOrtho ? '#22c55e' : (isDark ? '#38bdf8' : '#0284c7');
        ctx.lineWidth = 2.2;
        ctx.beginPath();
        ctx.arc(sHover.x, sHover.y, 14, 0, 2 * Math.PI);
        ctx.stroke();

        ctx.fillStyle = cadOrtho ? '#22c55e' : '#38bdf8';
        ctx.beginPath();
        ctx.arc(sHover.x, sHover.y, 3.5, 0, 2 * Math.PI);
        ctx.fill();

        ctx.lineWidth = 1.6;
        ctx.beginPath();
        ctx.moveTo(sHover.x, sHover.y - 18); ctx.lineTo(sHover.x, sHover.y - 14);
        ctx.moveTo(sHover.x, sHover.y + 14); ctx.lineTo(sHover.x, sHover.y + 18);
        ctx.moveTo(sHover.x - 18, sHover.y); ctx.lineTo(sHover.x - 14, sHover.y);
        ctx.moveTo(sHover.x + 14, sHover.y); ctx.lineTo(sHover.x + 18, sHover.y);
        ctx.stroke();

        // 2. Floating Touch HUD Badge (ลอยอยู่เหนือปลายนิ้ว 48px เสมอ เพื่อไม่ให้นิ้วบังมุมมอง)
        let hudY = sHover.y - 48;
        let hudText = `📍 (${cadHover.x.toFixed(2)}, ${cadHover.y.toFixed(2)})`;
        if (polyDrawingMode && polygonPoints.length > 0) {
            let last = polygonPoints[polygonPoints.length - 1];
            let dist = Math.hypot(cadHover.x - last.x, cadHover.y - last.y);
            let angleDeg = (Math.atan2(cadHover.y - last.y, cadHover.x - last.x) * 180 / Math.PI + 360) % 360;
            hudText = `📏 ${dist.toFixed(2)} ม. | ${angleDeg.toFixed(0)}° ${cadOrtho ? '🔒' : ''}`;
        }
        if (cadHover.snapType === 'guideEdgeIntersection') hudText += ` [✖ ไกด์]`;
        else if (cadHover.snapType === 'endpoint') hudText += ` [□ จุดมุม]`;
        else if (cadHover.snapType === 'midpoint') hudText += ` [▲ กึ่งกลาง]`;
        else if (cadHover.snapType === 'nearestEdge') hudText += ` [⧖ บนขอบ]`;

        ctx.font = 'bold 12px monospace';
        let tm = ctx.measureText(hudText);
        let bw = tm.width + 18;
        let bh = 24;
        ...
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.roundRect(sHover.x - bw / 2, hudY - bh / 2, bw, bh, 6);
        ctx.fill();
        ctx.stroke();

        // Arrow pointer pointing to reticle
        ctx.fillStyle = 'rgba(15, 23, 42, 0.94)';
        ctx.beginPath();
        ctx.moveTo(sHover.x - 5, hudY + bh / 2);
        ctx.lineTo(sHover.x, hudY + bh / 2 + 6);
        ctx.lineTo(sHover.x + 5, hudY + bh / 2);
        ctx.fill();

        ctx.fillStyle = '#f8fafc';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(hudText, sHover.x, hudY);
        ctx.restore();
    }
