...
function drawPolygonSheetArrow(ctx, x, yBottom, yTop, text, color = '#00c800', isDark = false, showLabel = true) {
    ...
    // หัวลูกศรล่างและบน
    let head = Math.min(6, arrowLen / 3);
    
    // ลูกศรบน (ชี้ขึ้น)
    ctx.beginPath();
    ctx.moveTo(x, yTop);
    ctx.lineTo(x - 3, yTop + head);
    ctx.lineTo(x + 3, yTop + head);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(x, yBottom);
    ctx.lineTo(x - 3, yBottom - head);
    ctx.lineTo(x + 3, yBottom - head);
    ctx.closePath();
    ctx.fill();

    // ตัวเลขบอกความยาวแผ่นตัด (หมุน 90 องศาแนวตั้ง)
    if (showLabel && arrowLen >= 20) {
        ...
        // พื้นหลังป้ายขนาดกะทัดรัด (ไม่ทับถมกันเป็นก้อนขาว)
        ctx.fillStyle = isDark ? 'rgba(18, 18, 20, 0.88)' : 'rgba(255, 255, 255, 0.90)';
        ctx.strokeStyle = isDark ? '#22c55e' : '#16a34a';
        ctx.lineWidth = 0.8;
        ctx.beginPath();
        ctx.roundRect(-tw / 2 - 3, -7, tw + 6, 14, 3);
        ctx.fill();
        ctx.stroke();
